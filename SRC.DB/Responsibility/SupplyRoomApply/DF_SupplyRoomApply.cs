...
namespace SRC.DB.Responsibility.SupplyRoomApply
{
    public class DF_SupplyRoomApply:ADF, IDF_SupplyRoomApply
    {
        public DF_SupplyRoomApply(IDataBaseFactory dataBaseService, EFContext db) : base(dataBaseService, db)
        {
        }
        ...
        public async Task Edit(long pid, string account)
        {
            var data = await DB.unit_applies.Where(x => x.pid == pid).FirstOrDefaultAsync();

            if(data == null)
            {
                throw new Exception($"查無供應室申請及撤銷作業資料，pid：{pid}");
            }
            ...
            unit_apply_review_log log = new unit_apply_review_log()
            {
                ...
                new_state = UNITAPPLY_STATE.STATE.DISTRIBUTE_OK.ToString(),
            ...
            data.state = UNITAPPLY_STATE.STATE.DISTRIBUTE_OK.ToString();
            data.editor = account;
        ...
        public async Task Cancel(List<long> pids,string account)
        {
            var data = await DB.unit_applies.Where(x => pids.Contains(x.pid)).ToListAsync();
            ...
            foreach(var each in data)
            {
                logs.Add(new unit_apply_review_log()
                {
                    ...
                    new_state = UNITAPPLY_STATE.STATE.SUPPLYROOM_CANCEL.ToString(),
                    create_time = DateTime.Now,
                    creator = account,
                    memo = ""
                });

                each.state = UNITAPPLY_STATE.STATE.SUPPLYROOM_CANCEL.ToString();
            }
